...
library(knitr)
kable(hanson_data[,c(2,4,5,6)], caption = "Data taken in Hanson Hall of Science", "latex")
```
...
```{r}
# Given data
n <- 16  # Sample size for each building
h_download_mean <- mean(hanson_data$`Download Speed (Mbps)`)
o_download_mean <- mean(oldmain_data$`Download Speed (Mbps)`)
h_download_sd <-  sd(hanson_data$`Download Speed (Mbps)`)
o_download_sd <- sd(oldmain_data$`Download Speed (Mbps)`)
...
$Calculation of Mean Difference and Standard Deviation$ The mean
difference and the standard deviation of the difference in download
speeds are calculated as follows:

```{r}
# Calculate the mean and standard deviation of the difference in download speeds
o_mean_diff <- h_download_mean - o_download_mean
o_sd_diff <- sqrt((h_download_sd^2 / n) + (o_download_sd^2 / n))
o_mean_diff
```

$mean_diff$ = 22.8325 $sd_diff$ = 30.15495

##Statistical Testing of Mean Difference of Download Speeds To test the
significance of the difference, we calculate the t-statistic and the
corresponding p-value:

```{r}
t_statistic <- o_mean_diff / o_sd_diff
df <- n - 1  # Degrees of freedom

# Calculate the p-value
p_value <- 2 * (1- pt(t_statistic,df))
p_value
```

$p_(value)$ = 0.4606757

##Meaning of P_value The p-value of 0.4606757 is greater than the
...
the true means of download speeds from Hanson Hall and Old Main Hall\$:
...
###Calculate the confidence interval for mean difference of download
speeds.

```{r}
# Calculate the margin of error
margin_error <- t_value * sd_diff

# Calculate the confidence interval
ci_lower <- o_mean_diff - margin_error
ci_upper <- o_mean_diff + margin_error
...
```{r}
u_mean_diff <- h_upload_mean - o_upload_mean
u_sd_diff <- sqrt((h_upload_sd^2 / n) + (o_upload_sd^2 / n))
t_statistic <- u_mean_diff / u_sd_diff
df <- n - 1  # Degrees of freedom
...
###To calculate the 95% confidence interval for the difference between
the true means of upload speeds from Hanson Hall and Old Main Hall:
...
# Calculate the margin of error
margin_error <- t_value * u_sd_diff

# Calculate the confidence interval
ci_lower <- u_mean_diff - margin_error
ci_upper <- u_mean_diff + margin_error
